...
const cardVariants = cva("rounded-xl border text-card-foreground transition-all duration-300", {
  variants: {
    variant: {
      ...
      gradient: "bg-gradient-to-b from-card to-background border-border/50 shadow-card",
      elevated: "bg-card border-border shadow-elevated hover:shadow-glow hover:border-primary/30",
      contest: "bg-gradient-to-b from-card to-background border-border/50 shadow-card hover:border-primary/50 hover:shadow-glow cursor-pointer"
    }
  },
  defaultVariants: {
    variant: "default"
...
const CardTitle = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({
  className,
  ...props
}, ref) => <div ref={ref} className={cn("text-xl font-semibold leading-none tracking-tight", className)} {...props} />);
